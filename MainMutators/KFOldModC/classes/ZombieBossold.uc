...
class ZombieBossold extends zombieboss;
...
simulated function CloakBoss()
{
	...
	Skins[4] = Shader'KF_Specimens_Trip_T.patriarch_invisible_gun';
	Skins[5] = Shader'KF_Specimens_Trip_T.patriarch_invisible';
    Skins[6] = Shader'KF_Specimens_Trip_T.patriarch_invisible_gun';
        Loopanim('runf',1.5);
...
simulated function AddTraceHitFX( vector HitPos )
{
	...
	local float hitDist;
	local ROBulletHitEffect H;

	PlaySound(SoundGroup'KFOldModSnd.Weapon.L85All',SLOT_Misc,2,,1400,0.9+FRand()*0.2);
	Start = GetBoneCoords('tip').Origin;
	if( mTracer==None )
		mTracer = Spawn(Class'KFNewTracer',,,Start);
	...
	Instigator = Self;
	H = Spawn(Class'ROBulletHitEffect',,,HitPos);
	if( H!=None )
...
defaultproperties
{
     RocketFireSound=Sound'KFOldModSnd.Weapon.LAWFire'
     MiniGunFireSound=Sound'KFOldModSnd.Weapon.L85-Loop'
     MiniGunSpinSound=None
     MGDamage=5.000000
     ClawMeleeDamageRange=50.000000
     ImpaleMeleeDamageRange=35.000000
     MoanVoice=SoundGroup'KFOldModSnd.Patriarch.PatMoan'
     ...
     ColHeight=40.000000
     PlayerCountHealthScale=0.000000
     HitSound(0)=SoundGroup'KFOldModSnd.Patriarch.PatHurt'
     ScoringValue=80
     RagShootStrength=6000.000000
     RagGravScale=2.000000
     HealthMax=8000.000000
     Health=8000
     ControllerClass=Class'KFOldModC.BossZombieControllerb'
     TurnLeftAnim="WalkF"
     TurnRightAnim="RunF"
