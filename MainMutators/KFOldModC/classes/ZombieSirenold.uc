...
class ZombieSirenold extends zombiesirenbase ;
...
{
	if( AnimName=='Siren_Scream' || AnimName=='Siren_Bite' || AnimName=='Siren_Bite' )
	{
	...
	if (Level.NetMode != NM_DedicatedServer)
	{
		...
		if (PC != None && PC.ViewTarget != None)
		{
			...
			if (Dist < ScreamRadius )
			{
				...
				if ( !KFGameType(Level.Game).bDidSirenScreamMessage && FRand() < 0.10 )
				{
					PC.Speech('AUTO', 16, "");
...
simulated function Tick( float Delta )
{
	...
	if( Role == ROLE_Authority )
	{
        if( bShotAnim )
        {
            GroundSpeed = OriginalGroundSpeed * 0.65;
...
defaultproperties
{
     ...
     Mesh=SkeletalMesh'KFCharactermodelsoldC.InfectedWhiteMale2'
     DrawScale=1.100000
