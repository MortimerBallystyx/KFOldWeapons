...
function bool IsInPounceDist(actor PTarget)
{
  ...
  time = vsize(DistVec)/ZombieCrawlerold(pawn).PounceSpeed;
  ...
  VSize(pawn.Location - PTarget.Location) < KFMonsterold(pawn).MeleeRange * 5)
    return true;
  else
    return false;
}
...
function bool FireWeaponAt(Actor A)
{
    ...
    else
    {
        ...
        if((LastPounceTime + (1.5 - (FRand() * 3.0))) < Level.TimeSeconds )
        {
            ...
            if ( RelativeDir > 0.85 )
            {
                ...
                if(IsInPounceDist(A) )
                {
                    if(ZombieCrawlerold(Pawn).DoPounce()==true )
...
{
  if( zombiecrawlerold(pawn).bPouncing )
  {
